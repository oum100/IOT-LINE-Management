...
<template>
  <div class="section-card border border-slate-800 bg-slate-950 p-4">
    <div class="flex items-center justify-between">
      <div>
        <p class="text-sm font-semibold uppercase tracking-[0.25em] text-orange-500">Step 3</p>
        <h3 class="text-lg font-semibold text-white">สรุปรายการ และยอดรวม</h3>
      </div>
    ...
    <div v-if="cart.items.length" class="mt-4 space-y-3">
      <div
        ...
        :key="item.cartId"
        class="flex items-center justify-between rounded-2xl border border-slate-800 bg-slate-900 px-3 py-3"
      >
        <div>
          <p class="font-medium text-slate-100">{{ item.machineName }}</p>
          <p class="text-sm text-slate-300">{{ item.priceLabel }} • {{ serviceText(item) }}</p>
        </div>
        <div class="flex items-center gap-3">
          <p class="font-semibold text-slate-100">{{ item.amount }} บาท</p>
    ...
    <div class="mt-4 flex items-center justify-between border-t border-slate-800 pt-4">
      ...
      <span class="text-2xl font-semibold text-teal-400">{{ total }} บาท</span>
    </div>

    <button
      type="button"
      class="mt-4 w-full rounded-xl bg-orange-500 px-4 py-2.5 text-base font-semibold !text-white transition hover:bg-orange-400 disabled:cursor-not-allowed disabled:opacity-60"
