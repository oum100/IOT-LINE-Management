...
async function loadData() {
  ...
  try {
    ...
    const [listRes, summaryRes] = await Promise.all([
      $fetch<PagingResponse<PaymentItem>>("/api/admin/payments", {
        query: {
          ...queryBase,
      ...
      $fetch<PaymentSummary>("/api/admin/payments/summary", { query: queryBase }),
    ])
    ...
    items.value = []
    total.value = 0
    error.value = (err as { data?: { statusMessage?: string }; message?: string })?.data?.statusMessage || "Failed to load payments"
  } finally {
    loading.value = false
  }
}
...
<template>
  <section class="space-y-4 text-slate-900 dark:text-slate-100">
    <div>
      <h1 class="text-2xl font-bold text-slate-900 dark:text-white">Payment Management</h1>
      <p class="mt-1 text-sm text-slate-500 dark:text-slate-400">View and search payments by tenant/merchant (brand)/branch</p>
    </div>
    ...
    <UCard :ui="{ root: 'bg-white/95 dark:bg-slate-900/90 ring-1 ring-slate-200 dark:ring-slate-700' }">
      ...
      <div class="grid gap-3 md:grid-cols-3 xl:grid-cols-5">
        ...
        <UCard :ui="{ root: 'bg-white dark:bg-slate-950/60 ring-1 ring-slate-200 dark:ring-slate-700' }"><p class="text-xs text-slate-500 dark:text-slate-400">Pending</p><p class="text-2xl font-bold text-amber-500">{{ summary.pendingCount }}</p></UCard>
        ...
        <UCard :ui="{ root: 'bg-white dark:bg-slate-950/60 ring-1 ring-slate-200 dark:ring-slate-700' }"><p class="text-xs text-slate-500 dark:text-slate-400">Verified</p><p class="text-2xl font-bold text-emerald-500">{{ summary.verifiedCount }}</p></UCard>
        <UCard :ui="{ root: 'bg-white dark:bg-slate-950/60 ring-1 ring-slate-200 dark:ring-slate-700' }"><p class="text-xs text-slate-500 dark:text-slate-400">Rejected</p><p class="text-2xl font-bold text-rose-500">{{ summary.rejectedCount }}</p></UCard>
      </div>
      ...
      <div class="mt-4 flex items-center justify-between text-sm">
        <p class="text-slate-500 dark:text-slate-400">Showing {{ items.length }} of {{ total }} payments</p>
