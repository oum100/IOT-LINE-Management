...
const setPromoAmount = ref<number | null>(null)
const setPromoTenantId = ref('')
const setPromoMerchantId = ref('')
const setPromoBranchId = ref('')
const isSettingModalScope = ref(false)
const rows = ref<OfferRow[]>([])
const selectedProductId = ref('')
const productList = ref<Array<{ id: string; code: string; name: string; kind: string; amount: number; quantity: string; bindings: number; active: boolean }>>([])
const options = ref<{ tenants: Array<{ id: string; code: string; name: string }>; merchants: Array<{ id: string; code: string; name: string; tenantId?: string }>; branches: Array<{ id: string; code: string; name: string; merchantAccountId?: string; tenantId?: string }>; types: string[] }>({ tenants: [], merchants: [], branches: [], types: [] })
...
const inputClass = 'text-slate-900 placeholder:text-slate-500 dark:text-slate-100 dark:placeholder:text-slate-400'
const inputUi = { base: 'bg-white ring-1 ring-slate-300 focus:ring-2 focus:ring-blue-500 dark:bg-slate-800 dark:ring-slate-500' }
...
const modalMerchantOptions = computed(() => options.value.merchants.filter(m => !setPromoTenantId.value || m.tenantId === setPromoTenantId.value))
const modalBranchOptions = computed(() => options.value.branches.filter(b => !setPromoMerchantId.value || b.merchantAccountId === setPromoMerchantId.value))
const modalTenantOptions = computed(() => options.value.tenants)
...
async function loadData() {
  ...
  error.value = ''
  try {
    const res = await $fetch<{ items: OfferRow[]; total: number; page: number; pageSize: number; productList: Array<{ id: string; code: string; name: string; kind: string; amount: number; quantity: string; bindings: number; active: boolean }>; options: typeof options.value }>('/api/admin/promotions', {
      query: { tab: tab.value, tenantId: tenantId.value || undefined, merchantAccountId: merchantAccountId.value || undefined, branchId: branchId.value || undefined, type: type.value || undefined, page: page.value, pageSize: pageSize.value }
    })
...
async function deleteUpcoming(id: string, scope: 'asset' | 'branch' = 'asset') {
  deletingId.value = id
  try {
    await $fetch(`/api/admin/promotions/${id}`, { method: 'DELETE', query: { scope } })
...
async function pausePromotion(id: string, scope: 'asset' | 'branch' = 'asset') {
  mutatingId.value = id
  try {
    await $fetch(`/api/admin/promotions/${id}/pause`, { method: 'POST', query: { scope } })
    const target = rows.value.find(item => item.id === id)
    if (target) {
      target.active = false
      target.reason = scope === 'branch' ? 'paused-by-admin-branch' : 'paused-by-admin'
...
async function resumePromotion(id: string, scope: 'asset' | 'branch' = 'asset') {
  mutatingId.value = id
  try {
    await $fetch(`/api/admin/promotions/${id}/resume`, { method: 'POST', query: { scope } })
    const target = rows.value.find(item => item.id === id)
    if (target) {
      target.active = true
      target.reason = scope === 'branch' ? 'resumed-by-admin-branch' : 'resumed-by-admin'
      target.updatedAt = new Date().toISOString()
    }
    if (scope === 'branch') await loadData()
  } catch (e) {
...
function openSetPromotion(productId: string, baseAmount: number) {
  isSettingModalScope.value = true
  setPromoProductId.value = productId
  setPromoAmount.value = Number(baseAmount || 0)
  setPromoTenantId.value = tenantId.value || ''
  setPromoMerchantId.value = merchantAccountId.value || ''
  setPromoBranchId.value = branchId.value || ''
  setPromoError.value = ''
  nextTick(() => {
    isSettingModalScope.value = false
  })
  setPromoOpen.value = true
}

function closeSetPromotion() {
...
async function submitSetPromotion(payload: {
  ...
  try {
    ...
    setPromoError.value = (e as { data?: { statusMessage?: string }; message?: string })?.data?.statusMessage || (e as Error).message || 'Failed to set promotion'
  } finally {
    setPromoSaving.value = false
  }
}

watch([tab, tenantId, merchantAccountId, branchId, type], () => {
  page.value = 1
})
watch([tab, tenantId, merchantAccountId, branchId, type, page], () => { void loadData() }, { immediate: true })
watch(tenantId, () => { merchantAccountId.value = ''; branchId.value = '' })
watch(merchantAccountId, () => { branchId.value = '' })
watch(setPromoTenantId, () => {
  if (isSettingModalScope.value) return
  setPromoMerchantId.value = ''
  setPromoBranchId.value = ''
})
watch(setPromoMerchantId, () => {
  if (isSettingModalScope.value) return
  setPromoBranchId.value = ''
})
</script>
...
<template>
  <section class="space-y-4">
    <UCard :ui="{ root: 'bg-white/95 dark:bg-slate-900/90 ring-1 ring-slate-200 dark:ring-slate-700' }">
      <template #header>
        <div class="flex items-center justify-between gap-3">
          <div>
            <h1 class="text-2xl font-bold text-slate-900 dark:text-white">Promotion Management</h1>
            <p class="mt-1 text-sm text-slate-500 dark:text-slate-400">Manage current, upcoming, and history promotions across platform scope.</p>
          </div>
          <UBadge color="primary" variant="soft">{{ total }} items</UBadge>
        </div>
      </template>
    </UCard>
      ...
      <template #header>
        <div class="grid gap-3 md:grid-cols-4 md:items-end">
          <div class="flex flex-col gap-1">
            ...
            <select v-model="tenantId" :class="selectClass">
              <option value="">All tenants</option>
              <option v-for="t in options.tenants" :key="t.id" :value="t.id">{{ tenantLabel(t) }}</option>
            </select>
          </div>
          <div class="flex flex-col gap-1">
            <label class="text-sm font-medium text-slate-600 dark:text-slate-300">Merchant</label>
            <select v-model="merchantAccountId" :class="selectClass">
    ...
    <UCard :ui="{ root: 'bg-white/95 dark:bg-slate-900/90 ring-1 ring-slate-200 dark:ring-slate-700' }">
        ...
        <div class="flex flex-wrap items-center justify-between gap-x-4 gap-y-1">
          <div class="flex flex-wrap items-center gap-x-4 gap-y-1">
            ...
            <p class="text-sm text-slate-500 dark:text-slate-400">
              Tenant:
              <span class="ml-1 font-semibold text-slate-800 dark:text-slate-100">{{ tenantId ? options.tenants.find(t => t.id === tenantId)?.name || '-' : 'All' }}</span>
            </p>
            <p class="text-sm text-slate-500 dark:text-slate-400">
              Merchant:
      ...
      <div class="max-h-56 overflow-auto rounded-lg border border-slate-200 dark:border-slate-700">
        <table class="w-full text-sm">
          ...
          <tbody>
            ...
            <tr v-for="p in productRows" :key="p.id" class="cursor-pointer border-t border-slate-200 dark:border-slate-700 hover:bg-slate-50/60 dark:hover:bg-slate-800/60" :class="selectedProductId === p.id ? 'bg-slate-100/70 dark:bg-slate-800/80' : ''" @click="selectedProductId = p.id">
              ...
              <td class="px-3 py-1.5 text-center">{{ p.bindings }}</td>
              <td class="px-3 py-1.5 text-center">
              ...
              <td class="px-3 py-1.5 text-center">
                <div class="flex items-center justify-center gap-1">
                  <UButton
                    ...
                    class="h-8 w-8"
                    title="Set Promotion"
                    @click.stop="openSetPromotion(p.id, p.amount)"
                  />
                  <UButton
    ...
    <SetPromotionModal
      ...
      :error="setPromoError"
      :asset="null"
      :product="setPromoProduct ? { id: setPromoProduct.id, code: setPromoProduct.code, name: setPromoProduct.name, amount: Number(setPromoAmount ?? setPromoProduct.amount ?? 0) } : null"
      :tenants="modalTenantOptions"
      :merchants="modalMerchantOptions"
      :branches="modalBranchOptions"
      :default-merchant-id="setPromoMerchantId"
      :default-branch-id="setPromoBranchId"
