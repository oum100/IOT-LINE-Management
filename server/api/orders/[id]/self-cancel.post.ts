...
import { z } from 'zod'
import { getMockOrder, updateMockOrder } from '../../../utils/mock-orders'
import { assertOrderBranchScope } from '../../../utils/order-branch-scope'
import { prisma } from '../../../utils/prisma'
import { verifyOrderSelfCancelToken } from '../../../utils/order-self-cancel'
...
export default defineEventHandler(async (event) => {
  ...
  try {
    ...
    if (!order.selfCancelTokenHash || !verifyOrderSelfCancelToken(body.token, order.selfCancelTokenHash)) {
      throw createError({ statusCode: 403, statusMessage: 'Invalid self-cancel token' })
    }
    ...
    if (!mockOrder) throw error

    if (!mockOrder.selfCancelTokenHash || !verifyOrderSelfCancelToken(body.token, mockOrder.selfCancelTokenHash)) {
      throw createError({ statusCode: 403, statusMessage: 'Invalid self-cancel token' })
    }
