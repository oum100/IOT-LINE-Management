...
export default defineEventHandler(async (event) => {
  await assertPermission(event, 'portal.merchant.manage')
  ...
  const code = await generateMerchantCode(resolvedTenantId, body.name)
  const created = await prisma.merchantAccount.create({
    data: {
